...
void CTempEnts::EjectBrass( const Vector &pos1, const QAngle &angles, const QAngle &gunAngles, int type )
{
	...
	if ( type == 1 )
	{
		pTemp->hitSound = BOUNCE_RIFLESHELL;
	}
	...
	{
		pTemp->hitSound = BOUNCE_REVOLVERSHELL;
	}
	
	else if ( type == 4 )
	{
		pTemp->hitSound = BOUNCE_FLARESHELL;
	}

	else if ( type == 10 )
	{
		pTemp->hitSound = BOUNCE_OICWMAG;
	}		
	
	else if ( type == 11 )
	{
		pTemp->hitSound = BOUNCE_SMG1MAG;
	}	
	
	else if ( type == 12 )
	{
		pTemp->hitSound = BOUNCE_PISTOLMAG;
	}	
	
	else if ( type == 13 )
	{
		pTemp->hitSound = BOUNCE_GRENADEMAG;
	}	
	...
	pTemp->die = gpGlobals->curtime + 4.0f + random->RandomFloat( 0.0f, 1.0f );	// Add an extra 0-1 secs of life	
}
...
void CTempEnts::PlaySound ( C_LocalTempEntity *pTemp, float damp )
{
	...
	case BOUNCE_REVOLVERSHELL:
		{
			soundname = "Bounce.RevolverShell";
			isshellcasing = true; // shell casings have different playback parameters
		}
		break;
		
	case BOUNCE_FLARESHELL:
		{
			soundname = "Bounce.FlareShell";
			isshellcasing = true; // shell casings have different playback parameters
		}
		break;
	...
	case BOUNCE_SMG1MAG:
		{
			soundname = "Bounce.SMG1Mag";
			isshellcasing = true; // shell casings have different playback parameters
		}
		break;
		
	case BOUNCE_PISTOLMAG:
		{
			soundname = "Bounce.PistolMag";
			isshellcasing = true; // shell casings have different playback parameters
		}
		break;
		
	case BOUNCE_GRENADEMAG:
		{
			soundname = "Bounce.GrenadePin";
			isshellcasing = true; // shell casings have different playback parameters
		}
		break;
...
void CTempEnts::LevelInit()
{
	...
	m_pShells[3] = (model_t *) engine->LoadModel( "models/weapons/revolver_shell.mdl" );
	m_pShells[4] = (model_t *) engine->LoadModel( "models/weapons/flare_shell.mdl" );
	m_pShells[10] = (model_t *) engine->LoadModel( "models/weapons/oicw_magazine.mdl" );
	m_pShells[11] = (model_t *) engine->LoadModel( "models/weapons/smg1_magazine.mdl" );
	m_pShells[12] = (model_t *) engine->LoadModel( "models/weapons/pistol_magazine.mdl" );
	m_pShells[13] = (model_t *) engine->LoadModel( "models/weapons/grenade_magazine.mdl" );
#endif
...
#define SHELLTYPE_REVOLVER	3
#define SHELLTYPE_FLARE		4
