...
void OnCaptionLanguageChanged( IConVar *pConVar, const char *pOldString, float flOldValue )
{
	...
	Q_snprintf(fn2, sizeof(fn2), "resource/raisingthebarredux_%s.txt", var.GetString());
	Q_snprintf(fn3, sizeof(fn3), "resource/gameui_%s.txt", var.GetString());

	// Re-adding the file, even if it's "english" will overwrite the tokens as needed
	if ( !IsX360() )
	{
		g_pVGuiLocalize->AddFile( "resource/closecaption_%language%.txt", "GAME", true );
		g_pVGuiLocalize->AddFile("resource/raisingthebarredux_%language%.txt", "GAME", true);
	...
	if ( Q_strlen( var.GetString() ) > 0 && Q_stricmp( var.GetString(), uilanguage ) )
	{
		if ( !IsX360() )
		{
			...
			if (g_pFullFileSystem->FileExists(fn2))
			{
				char dbfile[512];
				Q_snprintf(dbfile, sizeof(dbfile), "resource/raisingthebarredux_%s.txt", var.GetString());
				g_pVGuiLocalize->AddFile(fn2, "GAME", true);
				hudCloseCaption->InitCaptionDictionary(dbfile);
			}
			else
			{
				char fallback[512];
				Q_snprintf(fallback, sizeof(fallback), "resource/raisingthebarredux_%s.txt", uilanguage);

				Msg("%s not found\n", fn2);
				Msg("%s will be used\n", fallback);
			}
			if (g_pFullFileSystem->FileExists(fn3))
			{
				char dbfile[512];
				Q_snprintf(dbfile, sizeof(dbfile), "resource/gameui_%s.txt", var.GetString());
				g_pVGuiLocalize->AddFile(fn3, "GAME", true);
				hudCloseCaption->InitCaptionDictionary(dbfile);
			}
			else
			{
				char fallback[512];
				Q_snprintf(fallback, sizeof(fallback), "resource/gameui_%s.txt", uilanguage);
