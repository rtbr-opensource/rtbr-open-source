...
void CBaseEntity::TakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	else
	{
		...
		// Hack to get afterburn working.
		if (GetFlags() & FL_ONFIREGREEN && info.GetDamageType() == (DMG_BURN | DMG_DIRECT))
		{
			info.SetDamage( roundf( 1 * GetReceivedDamageScale( info.GetAttacker() ) ) );
		}
		OnTakeDamage( info );
	}
}
...
//	DEFINE_FIELD( m_ToolHandle,		FIELD_INTEGER ),
	...
	DEFINE_FIELD(m_bSecondaryUse, FIELD_BOOLEAN),
