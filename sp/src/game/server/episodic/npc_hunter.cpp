...
ConVar hunter_lr_attack_volley_size( "hunter_lr_attack_volley_size", "12" );
...
ConVar hunter_lr_first_attack_delay( "hunter_lr_first_attack_delay", "3.0" );
...
ConVar sk_hunter_lr_buckshot_damage_scale( "sk_hunter_lr_buckshot_damage_scale", "0" );
...
//-----------------------------------------------------------------------------
// Purpose: used in npc_crabsynth as of now
//-----------------------------------------------------------------------------
...
public:
	...
	int				GetHunterRangeAttack2Schedule() { return m_bIsAlt ? SCHED_HUNTER_LR_RANGE_ATTACK2 : SCHED_HUNTER_RANGE_ATTACK2; }
...
private:
	...
	void ShootMinigun(const Vector* pTarget, float aimError, const Vector& vecSpread);
	void TraceHitscanAttack(const CTakeDamageInfo& inputInfo, const Vector& vecDir, trace_t* ptr, CDmgAccumulator* pAccumulator);
...
LINK_ENTITY_TO_CLASS(npc_hunter_lr, CNPC_Hunter);
...
DEFINE_FIELD(m_bIsAlt, FIELD_BOOLEAN),
...
void CNPC_Hunter::Precache()
{
	...
	if (m_bIsAlt)
	{
		PrecacheScriptSound( "NPC_Hunter_LR.WindUp" );
		PrecacheScriptSound( "NPC_Hunter_LR.Shoot" );
	}
...
{
	if (FClassnameIs(this, "npc_hunter_lr")) {
		m_bIsAlt = true;
	}
	else {
		m_bIsAlt = false;
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Hunter::ShootMinigun(const Vector* pTarget, float aimError, const Vector& vecSpread)
{
	if (pTarget)
	{
		Vector muzzlePos;
		...
		GetAttachment("MiniGunBase", muzzlePos, muzzleAng);

		Vector vecShootDir = *pTarget - muzzlePos;
		VectorNormalize(vecShootDir);

		// exactly on target w/tracer
		FireBullets(1, muzzlePos, vecShootDir, vecSpread, 8192, GetAmmoDef()->Index("StriderMinigun"), 1);

		trace_t tr;

		UTIL_TraceLine(muzzlePos, *pTarget, MASK_SHOT, this, COLLISION_GROUP_NONE, &tr);



		g_pEffects->MuzzleFlash( muzzlePos, muzzleAng, random->RandomFloat( 2.0f, 4.0f ) , MUZZLEFLASH_TYPE_STRIDER );

		DispatchParticleEffect("weapon_tracer_pulse_heavy", muzzlePos, tr.endpos, muzzleAng);

		if (!m_bIsAlt)
			EmitSound( "NPC_Hunter.FireMinigun" );
		else
			EmitSound( "NPC_Hunter_LR.Shoot" );
	}
}
...
//---------------------------------------------------------
//---------------------------------------------------------
/*void CNPC_Hunter::TraceHitscanAttack(const CTakeDamageInfo& inputInfo, const Vector& vecDir, trace_t* ptr, CDmgAccumulator* pAccumulator)
{
...
void CNPC_Hunter::BuildScheduleTestBits()
{
	...
	// If we're a long range hunter, we cannot be interrupted while attacking.
	if (IsCurSchedule( SCHED_HUNTER_LR_RANGE_ATTACK2, false ))
	{
		ClearCustomInterruptConditions();
	}
	else if (IsCurSchedule(SCHED_HUNTER_RANGE_ATTACK2, false) && (gpGlobals->curtime < m_flShootAllowInterruptTime))
	{
...
int CNPC_Hunter::SelectCombatSchedule()
{
	...
	if (hunter_flechette_test.GetBool())
	{
		if (HasCondition(COND_CAN_RANGE_ATTACK2))
		{
			...
			return GetHunterRangeAttack2Schedule();
		}
...
int CNPC_Hunter::TranslateSchedule(int scheduleType)
{
	...
	case SCHED_HUNTER_LR_RANGE_ATTACK2:
...
void CNPC_Hunter::StartTask(const Task_t* pTask)
{
	...
	case TASK_RANGE_ATTACK2:
	{
		if (GetEnemy())
		{
			...
			int nShots = m_bIsAlt ? hunter_lr_attack_volley_size.GetInt() : hunter_flechette_volley_size.GetInt();
			...
			if (m_bIsAlt)
			{
				initialDelay = hunter_lr_first_attack_delay.GetFloat();
				EmitSound( "NPC_Hunter_LR.WindUp" );
			}
...
void CNPC_Hunter::TraceAttack(const CTakeDamageInfo& inputInfo, const Vector& vecDir, trace_t* ptr, CDmgAccumulator* pAccumulator)
{
		...
		(info.GetDamageType() & DMG_NEVERGIB))
	{
		...
		if (info.GetDamageType() & DMG_BUCKSHOT)
		{
			...
			if (m_bIsAlt)
			{
				// Further decrease the damage dealt by buckshot.
				flScale *= sk_hunter_lr_buckshot_damage_scale.GetFloat();
			}
		}
...
int CNPC_Hunter::CountRangedAttackers()
{
	...
	for (int i = 0; i < g_Hunters.Count(); i++)
	{
		...
		{
			if (pOtherHunter->IsCurSchedule(SCHED_HUNTER_RANGE_ATTACK2) || pOtherHunter->IsCurSchedule(SCHED_HUNTER_LR_RANGE_ATTACK2))
			{
				nAttackers++;
			}
		}
...
bool CNPC_Hunter::ShootFlechette(CBaseEntity* pTargetEntity, bool bSingleShot)
{
	...
	int nShots = m_bIsAlt ? hunter_lr_attack_volley_size.GetInt() : hunter_flechette_volley_size.GetInt();
	int nShotNum = nShots - m_nFlechettesQueued;
	...
	if (m_bIsAlt) {
		vecShoot = manipulator.ApplySpread(VECTOR_CONE_1DEGREES, 1.0f);
	}
	...
	if (!m_bIsAlt) {
		...
		ShootMinigun(&(pTargetEntity->WorldSpaceCenter()), 0.3, VECTOR_CONE_4DEGREES);
	}
