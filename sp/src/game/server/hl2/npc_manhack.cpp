...
void CNPC_Manhack::Event_Killed( const CTakeDamageInfo &info )
{
	...
	DispatchParticleEffect("npc_manhack_explosion", PATTACH_POINT_FOLLOW, this, "Light", true);
...
bool CNPC_Manhack::CorpseGib( const CTakeDamageInfo &info )
{
	...
	DispatchParticleEffect("npc_manhack_explosion", GetAbsOrigin(), GetAbsAngles());
	if( info.GetDamageType() & DMG_CLUB )
	{
