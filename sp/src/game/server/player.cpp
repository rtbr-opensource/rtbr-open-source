...
void CBasePlayer::ResetFidget(void)
{
...
Vector CBasePlayer::CenterBodyTarget(const Vector &posSrc, bool bNoisy)
{
	if (IsInAVehicle())
	{
		return GetVehicle()->GetVehicleEnt()->BodyTarget(posSrc, bNoisy);
	...
	if (bNoisy)
	{
		return GetAbsOrigin() + ((GetViewOffset() - Vector(0, 0, 32)) * random->RandomFloat(0.7, 1.0));
	}
	else
	{
		return EyePosition();
