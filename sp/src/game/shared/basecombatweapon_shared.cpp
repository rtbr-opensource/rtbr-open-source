...
#ifdef RTBR_DLL
const CRTBRWeaponInfo &CBaseCombatWeapon::GetRTBRWpnData( void ) const
{
	return *static_cast< CRTBRWeaponInfo* >( GetFileWeaponInfoFromHandle( m_hWeaponFileInfo ) );
}
#endif
...
{
	if( m_iAltFireHudHintCount >= WEAPON_ALTFIRE_HUD_HINT_COUNT )
		return false;
...
bool CBaseCombatWeapon::Reload(void)
{
	...
	if (m_iClip1 == 0 && !m_bNoEmptyReload) {
...
bool CBaseCombatWeapon::SetIdealActivity( Activity ideal )
{
	...
	if ( ideal != NULL && IsWeaponVisible() && nextSequence != m_nIdealSequence )
	{
...
BEGIN_DATADESC( CBaseCombatWeapon )
	...
	DEFINE_FIELD( m_bInReload, FIELD_BOOLEAN ),
	DEFINE_FIELD(m_bNoEmptyReload, FIELD_BOOLEAN),
	DEFINE_FIELD( m_bFireOnEmpty, FIELD_BOOLEAN ),
